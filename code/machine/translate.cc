...
// from the virtual address
    ...
    if (tlb == NULL) {    // => page table => vpn is index into table
      ...
      {
        if (pageTable[i].virtualPage == vpn  && pageTable[i].procId == kernel->currentThread->id)
        {
          // cout<<vpn<<" "<<kernel->currentThread->id<<"FOUND in PAGETABLE\n";
          entry = &pageTable[i];
      ...
      for (entry = NULL, i = 0; i < TLBSize; i++)
        if (tlb[i].valid && (tlb[i].virtualPage == ((int)vpn)) && (tlb[i].procId == kernel->currentThread->id)) {
          cout<<"FOUND in TLB\n";
          entry = &tlb[i];      // FOUND!
          ppn = entry->physicalPage;
