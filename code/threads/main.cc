...
RunUserProg(void *filename) {
    AddrSpace *space = new AddrSpace((char*)filename);
    // AddrSpace *space = new AddrSpace();
    ASSERT(space != (AddrSpace *)NULL);
    cout<< "User program "<< filename <<" running"<<endl;
...
main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        userPrograms.push_back(userProgName);
    ...
    //     for (vector<char*>::iterator i = userPrograms.begin(); i != userPrograms.end(); ++i) {
    //         cout << "User Program" << *i<<" Forked"<<endl;
    //         Thread *t = new Thread(*i);
    //         t->Fork((VoidFunctionPtr)RunUserProg, (void*)*i);
    //     }


    for (int i =0; i<userPrograms.size(); i++)
    {
        Thread *tr = new Thread(userPrograms[i]);
        tr->Fork((VoidFunctionPtr)RunUserProg, (char *)userPrograms[i]);
        cout<<"Thread "<<userPrograms[i]<<" Had been forked\n";
    }
